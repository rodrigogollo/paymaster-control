...
import {
  ...
  STATUS_VALUES,
  type NewProject,
} from "../../src/db/schema/project.schema.ts";
import {
  timeEntities,
  type NewTimeEntity,
} from "../../src/db/schema/timeEntity.schema.ts";
...
export async function createDemoProject(availableClientIds: string[]) {
  ...
  const projectEndDate = faker.date.soon({
    days: 365,
    refDate: projectStartDate,
  });
...
export async function createDemoTimeEntity(
  userIds: string[],
  projectIds: string[],
) {
